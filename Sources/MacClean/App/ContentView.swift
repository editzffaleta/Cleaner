...
struct ContentView: View {
    ...
    var body: some View {
        ...
        NavigationSplitView {
            ...
            ZStack {
                ...
                if appState.selectedSidebarItem == nil {
                    Text(L10n.tr("请从侧边栏选择一个模块", "Selecione um módulo na barra lateral"))
        ...
        .alert(
            L10n.tr("发现新版本", "Atualização disponível"),
            isPresented: Binding(
            ...
            case .brewCommand:
                Button(L10n.tr("复制升级命令", "Copiar Comando de Atualização")) {
                    updateCoordinator.performPrimaryAction()
                }
            case .openRelease:
                Button(L10n.tr("下载", "Baixar")) {
                    updateCoordinator.performPrimaryAction()
                }
            }
            Button(L10n.tr("跳过此版本", "Pular Esta Versão")) {
                updateCoordinator.skip(pending.version)
            }
            Button(L10n.tr("稍后", "Depois"), role: .cancel) {
