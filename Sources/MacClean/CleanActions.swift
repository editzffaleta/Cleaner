            ...
            do {
                ...
                for (path, msg) in r.perBinaryErrors {
                    errors.append(CleaningEngine.CleanError(
                        path: path, error: L10n.tr("二进制精简失败：\(msg)", "falha ao reduzir o binário: \(msg)")
                    ))
                }
            } catch {
                errors.append(CleaningEngine.CleanError(
                    path: item.url.path(percentEncoded: false),
                    error: L10n.tr("应用包精简失败：\(error.localizedDescription)", "falha ao reduzir o bundle: \(error.localizedDescription)")
                ))
            }
        }
        return CleaningEngine.CleanResult(
