...
public actor CleaningEngine {
    ...
    public func clean(
        ...
        if items.count > MCConstants.maxTotalItemsPerCleanOperation {
            let msg = L10n.tr("操作超过 \(MCConstants.maxTotalItemsPerCleanOperation) 项上限（尝试处理：\(items.count) 项）。这通常表示扫描器存在异常。", "A operação excede o limite de \(MCConstants.maxTotalItemsPerCleanOperation) itens (tentados: \(items.count)). Isso normalmente indica um bug no scanner.")
        ...
        for item in chunk {
            ...
            case .trash:
                do {
                    ...
                    skippedCount += 1
                    logSkip(path: item.url, reason: L10n.tr("已不存在", "já removido"))
                } catch {
