...
struct SettingsPageView: View {
        ...
        Section {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(L10n.tr("版本 \(MCConstants.appVersion)", "Versão \(MCConstants.appVersion)"))
                        .font(.caption)
            ...
            Toggle(L10n.tr("自动检查更新", "Verificar atualizações automaticamente"),
                   isOn: $automaticUpdateChecks)
            if lastUpdateCheckTimestamp > 0 {
                Text(L10n.tr(
                    "上次检查：\(lastCheckedDescription)",
                    "Última verificação: \(lastCheckedDescription)"))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private var updateControl: some View {
        switch updateState {
        case .idle:
            Button(L10n.tr("检查更新", "Verificar Atualizações")) { startUpdateCheck() }
        case .checking:
            HStack(spacing: 6) {
                ProgressView().controlSize(.small)
                Text(L10n.tr("正在检查…", "Verificando…")).foregroundStyle(.secondary)
            }
        case .upToDate:
            HStack(spacing: 4) {
                Image(systemName: "checkmark.circle.fill").foregroundStyle(.green)
                Text(L10n.tr("已是最新", "Atualizado"))
            }
        case .available:
        ...
        case .failed(let message):
            HStack(spacing: 6) {
                ...
                Button(L10n.tr("重试", "Tentar de novo")) { startUpdateCheck() }
            }
        }
    }
        ...
        if UpdateChecker.isHomebrewInstall() {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(L10n.tr("发现版本 \(version)。请使用 Homebrew 更新：", "A versão \(version) está disponível. Atualize com o Homebrew:"))
                ...
                Button {
                    ...
                    Label(L10n.tr("复制", "Copiar"), systemImage: "doc.on.doc")
                }
            }
        } else {
            HStack {
                Text(L10n.tr("发现版本 \(version)。", "A versão \(version) está disponível."))
                Spacer()
                Button(L10n.tr("查看发布页", "Ver Versão")) { NSWorkspace.shared.open(url) }
    ...
    private var generalSection: some View {
        Section(L10n.tr("通用", "Geral")) {
            // Toggle flips instantly; the SMAppService round-trip runs in the
            // background with a spinner (no main-thread block, no lag).
            HStack(spacing: 10) {
                VStack(alignment: .leading, spacing: 2) {
                    Text(L10n.tr("登录时启动", "Abrir no login"))
                    Text(L10n.tr("登录 macOS 时自动打开 \(MCConstants.appName)。你也可以在“系统设置 → 通用 → 登录项”中管理。", "Abrir o \(MCConstants.appName) automaticamente quando você entrar no macOS. Você também pode gerenciar isso em Ajustes do Sistema → Geral → Itens de Início."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer()
                ...
                Toggle(L10n.tr("登录时启动", "Abrir no login"), isOn: $launchAtLogin)
                    .toggleStyle(.switch)
                    .labelsHidden()
            ...
            if loginLauncher.status == .requiresApproval {
                Label(L10n.tr("需要在“系统设置 → 通用 → 登录项”中批准", "Precisa de aprovação em Ajustes do Sistema → Geral → Itens de Início"),
            ...
            HStack(spacing: 10) {
                VStack(alignment: .leading, spacing: 2) {
                    Text(L10n.tr("在菜单栏显示 \(MCConstants.appName)", "Mostrar o \(MCConstants.appName) na barra de menus"))
                    Text(L10n.tr("在屏幕顶部实时显示 CPU、内存、磁盘、电池和网络状态。点击可展开浮窗。", "CPU, memória, disco, bateria e rede ao vivo no topo da tela. Clique para expandir o popover."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer()
                ...
                Toggle(L10n.tr("在菜单栏显示 \(MCConstants.appName)", "Mostrar o \(MCConstants.appName) na barra de menus"), isOn: $showMenuBarWidget)
                    .toggleStyle(.switch)
                    .labelsHidden()
    ...
    private var interfaceLanguageSection: some View {
        Section(L10n.tr("界面语言", "Idioma da Interface")) {
            Picker(L10n.tr("语言", "Idioma"), selection: $appLanguageRaw) {
            ...
            Text(L10n.tr("切换后会立即应用到主界面和菜单栏小组件。", "As alterações se aplicam imediatamente à janela principal e ao widget da barra de menus."))
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
    ...
    private var appearanceSection: some View {
        Section(L10n.tr("外观", "Aparência")) {
            Picker(L10n.tr("主题", "Tema"), selection: $appearanceRaw) {
            ...
            HStack(spacing: 10) {
                VStack(alignment: .leading, spacing: 2) {
                    Text(L10n.tr("移除背景颜色", "Remover cores de fundo"))
                    Text(L10n.tr("将模块渐变背景替换为中性的深色背景，以提升可读性。", "Substituir os fundos em gradiente dos módulos por uma cor escura neutra para melhor legibilidade."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer()
                Toggle(L10n.tr("移除背景颜色", "Remover cores de fundo"), isOn: $removeBackgroundColors)
                    .toggleStyle(.switch)
                    .labelsHidden()
            }
        }
    ...
    private var languageSection: some View {
        Section(L10n.tr("语言清理", "Limpeza de Idiomas")) {
            Text(L10n.tr("英文、基础资源和中文会始终保留。已勾选的语言会保留；未勾选的语言文件可由“系统垃圾”移除。", "Inglês, recursos Base e chinês são sempre mantidos. Os idiomas marcados são preservados; os arquivos de idioma desmarcados podem ser removidos pelo Lixo do Sistema."))
                .font(.caption)
                .foregroundStyle(.secondary)

            if selectable.isEmpty {
                Text(L10n.tr("正在检测已安装语言…", "Detectando idiomas instalados…"))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            } else {
                TextField(L10n.tr("搜索语言", "Buscar idiomas"), text: $languageSearch)
                    .textFieldStyle(.roundedBorder)

                if filteredLanguages.isEmpty {
                    Text(L10n.tr("没有语言匹配“\(languageSearch)”。", "Nenhum idioma corresponde a “\(languageSearch)”."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
    ...
    private var aboutSection: some View {
        Section(L10n.tr("关于", "Sobre")) {
            aboutRow(icon: "chevron.left.forwardslash.chevron.right", tint: .orange,
                     title: L10n.tr("源代码", "Código-fonte"), caption: L10n.tr("在 GitHub 上浏览代码库", "Explore o código no GitHub"),
                     url: MCConstants.repoURL)
            aboutRow(icon: "exclamationmark.bubble", tint: .blue,
                     title: L10n.tr("报告问题", "Relatar um problema"), caption: L10n.tr("提交错误报告和功能请求", "Relatos de bugs e pedidos de recursos"),
                     url: MCConstants.issuesURL)
            aboutRow(icon: "tag", tint: .green,
                     title: L10n.tr("发行说明", "Notas da versão"), caption: L10n.tr("更新日志和历史版本", "Histórico de alterações e versões anteriores"),
    ...
    private var widgetStatusRow: some View {
        HStack {
            ...
            Text(L10n.tr("组件状态：", "Status do widget:"))
                .foregroundStyle(.secondary)
            Text(statusText)
                .font(.system(.body, design: .monospaced))
            Spacer()
    ...
    private var statusText: String {
        ...
        case .notRegistered: return L10n.tr("未注册", "não registrado")
        case .notFound: return L10n.tr("未在应用包中找到辅助组件", "helper não encontrado no bundle")
        case .requiresApproval: return L10n.tr("需要在“系统设置 → 登录项”中批准", "precisa de aprovação em Ajustes do Sistema → Itens de Início")
