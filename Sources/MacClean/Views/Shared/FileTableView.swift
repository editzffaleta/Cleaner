...
struct FileTableView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        let reveal = NSMenuItem(
            title: L10n.tr("在 Finder 中显示", "Mostrar no Finder"),
...
private final class HeaderCellView: NSTableCellView {
    ...
    func configure(
        ...
        chevron.image = NSImage(
            ...
            accessibilityDescription: header.isExpanded ? L10n.tr("折叠", "Recolher") : L10n.tr("展开", "Expandir")
        )
        ...
        count.stringValue = L10n.tr("已选择 \(header.selectedCount)/\(header.fileCount)", "\(header.selectedCount)/\(header.fileCount) selecionados")
...
private final class ItemCellView: NSTableCellView {
    ...
    private let badge = BadgeView(text: L10n.tr("应用已打开", "App aberto"))
    private let size = NSTextField(labelWithString: "")
    ...
    init() {
        ...
        configureIconButton(infoButton, symbol: "info.circle", tooltip: L10n.tr("复制路径", "Copiar caminho"), action: #selector(infoClicked))
        configureIconButton(revealButton, symbol: "folder", tooltip: L10n.tr("在 Finder 中显示", "Mostrar no Finder"), action: #selector(revealClicked))
