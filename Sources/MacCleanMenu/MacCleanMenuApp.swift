...
struct MenuContentView: View {
    ...
    private var header: some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                Text(L10n.tr("实时系统状态", "Estatísticas do sistema ao vivo"))
                    .font(.system(size: 10))
        ...
        return LazyVGrid(columns: [GridItem(.flexible(), spacing: 10), GridItem(.flexible(), spacing: 10)], spacing: 10) {
            ...
            if let level = s.batteryLevel {
                ringCard(icon: s.batteryIsCharging ? "battery.100.bolt" : "battery.75",
                         label: s.batteryIsCharging ? L10n.tr("充电中", "Carregando") : L10n.tr("电池", "Bateria"),
    ...
    private func uptimeCard(_ s: SystemStatsCollector.SystemStats) -> some View {
        VStack(spacing: 12) {
            statHeader(icon: "clock", label: L10n.tr("运行时间", "Tempo ativo"), tint: MenuPalette.teal)
    ...
    private var recommendationsCard: some View {
        VStack(alignment: .leading, spacing: 9) {
            sectionHeader(icon: "sparkles", title: L10n.tr("建议", "Recomendações"), tint: MenuPalette.yellow)
    ...
    private func protectionCard(_ p: SharedAppState.ProtectionStatus) -> some View {
        ...
        return HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(p.threatsFound > 0 ? L10n.tr("发现 \(p.threatsFound) 个威胁", "\(p.threatsFound) threat\(p.threatsFound == 1 ? "" : "s") found") : L10n.tr("已防护", "Protegido"))
                    .font(.system(size: 12, weight: .semibold)).foregroundStyle(.white)
                Text(L10n.tr("上次扫描：\(relativeTime(p.lastScanDate)) · \(p.scanDepth)", "Escaneado \(relativeTime(p.lastScanDate)) · \(p.scanDepth)"))
    ...
    private func devicesCard(_ d: ConnectedDevices) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionHeader(icon: "externaldrive.connected.to.line.below", title: L10n.tr("已连接", "Conectado"), tint: MenuPalette.teal)
            ForEach(d.externalVolumes) { v in
                HStack(spacing: 7) {
                    ...
                    Spacer(minLength: 6)
                    Text(L10n.tr("\(FileSizeFormatter.format(v.freeBytes)) 可用", "\(FileSizeFormatter.format(v.freeBytes)) livres")).font(.system(size: 10, design: .monospaced)).foregroundStyle(MenuPalette.textSecondary)
                }
            }
    ...
    private var footer: some View {
        HStack(spacing: 8) {
            ...
            }.buttonStyle(.plain).help(L10n.tr("退出监视器", "Encerrar Monitor"))

            Button { TipAction.open() } label: {
                HStack(spacing: 6) {
                    Image(systemName: "leaf.fill").font(.system(size: 11, weight: .bold))
                    Text(L10n.tr("打开 \(MCConstants.appName)", "Abrir o \(MCConstants.appName)")).font(.system(size: 12, weight: .bold))
                }
                .foregroundStyle(.white)
    ...
    private func relativeTime(_ date: Date) -> String {
        ...
        if i < 60 { return L10n.tr("刚刚", "agora mesmo") }
        if i < 3600 { return L10n.tr("\(Int(i/60)) 分钟前", "há \(Int(i/60)) min") }
        if i < 86400 { return L10n.tr("\(Int(i/3600)) 小时前", "há \(Int(i/3600)) h") }
