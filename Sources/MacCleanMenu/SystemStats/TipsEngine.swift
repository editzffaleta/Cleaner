...
public actor TipsEngine {
    ...
    private func trashTip() async -> Tip? {
        ...
        return Tip(
            id: "trash_large",
            title: L10n.tr("废纸篓占用了 \(FileSizeFormatter.format(size))", "A Lixeira está ocupando \(FileSizeFormatter.format(size))"),
            body: L10n.tr("通过 \(MCConstants.appName) 清空废纸篓以释放空间。", "Esvazie a Lixeira pelo \(MCConstants.appName) para recuperar espaço."),
    ...
    private func cachesTip() async -> Tip? {
        ...
        return Tip(
            id: "caches_large",
            title: L10n.tr("用户缓存已增长到 \(FileSizeFormatter.format(size))", "Os caches do usuário cresceram para \(FileSizeFormatter.format(size))"),
            body: L10n.tr("在 \(MCConstants.appName) 中运行“系统垃圾”以清理可再生成的缓存文件。", "Execute o Lixo do Sistema no \(MCConstants.appName) para limpar arquivos de cache recriáveis."),
            estimatedSavings: size,
