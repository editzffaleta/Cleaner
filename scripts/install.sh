#!/bin/bash
# One-line installer for Cleaner.
# Usage: curl -fsSL https://raw.githubusercontent.com/editzffaleta/Cleaner/main/scripts/install.sh | bash

set -euo pipefail

REPO="editzffaleta/Cleaner"
...
if [ "$OS_VERSION" -lt 14 ]; then
    red "Cleaner requires macOS 14 (Sonoma) or later. You have $(sw_vers -productVersion)."
    exit 1
fi
...
cyan "Downloading Cleaner $VERSION..."
TMP=$(mktemp -d)
...
# Cleaner is notarized by Apple, so no quarantine workaround is needed.

green ""
green "✓ Cleaner $VERSION installed to $INSTALL_DIR/$APP_NAME"
green ""
